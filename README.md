...
### Configuration
You can adjust simulation parameters in real-time using the GUI panel (top-right corner).

**Interaction & Forces**
* **`isMouse`**: Switches the interaction mode. If checked, uses the mouse cursor; if unchecked, uses webcam pose estimation.
* **`osc_strength`**: Controls the intensity of the **Harmonic Function** force generated by body movements.
* **`n_strength`**: Controls the intensity of the background **Simplex Noise** turbulence.
* **`buoyancy`**: Adjusts the upward force applied to the fluid (how fast the smoke rises).
* **`mouse_force`**: Sets the magnitude of the external force when using mouse interaction.
* **`cursor_size`**: Controls the radius of the force/smoke emission (applies to both mouse and skeleton joints).

**Fluid Solver Settings**
* **`isViscous`**: Enables or disables the viscosity diffusion step.
* **`viscous`**: Sets the viscosity coefficient. Higher values make the fluid thicker (resist flow).
* **`iterations_viscous`**: Number of solver iterations for the diffusion step. Higher values are more accurate but computationally expensive.
* **`iterations_poisson`**: Number of solver iterations for the pressure projection step (ensures incompressibility).
* **`dt`**: The time step size (delta time). Controls the simulation speed.
* **`BFECC`**: Enables **Back and Forth Error Compensation and Correction**. This reduces numerical dissipation, keeping the smoke details sharper.

## 📷 Troubleshooting: Camera Access

If the camera permission popup does not appear or you cannot start the real-time interaction, please check the following steps. This issue is typically related to **browser security policies** rather than a software error.

### 1. Run in a Secure Context (HTTPS or localhost)
Modern web browsers enforce strict security policies for the `getUserMedia()` API. Camera access is **blocked** if the site is served over an insecure connection.

| Environment | Status | Reason |
| :--- | :--- | :--- |
| `file://` (Opening HTML directly) | ❌ **Not Supported** | Browsers treat local files as insecure. |
| `http://your-server.com` | ❌ **Not Supported** | Plain HTTP is insecure. |
| `https://your-domain.com` | ✅ **Supported** | Secure context. |
| `http://localhost:PORT` | ✅ **Supported** | Localhost is treated as a secure context. |

**Recommended Local Setup:**
If you are testing locally, do not double-click the `index.html` file. Instead, run a simple local server:

### 2. Manually Enable Permissions
If you previously denied camera access, the browser will likely block future requests silently. To reset this:
1.  Click the **Lock icon (🔒)** next to the URL address bar.
2.  Find **Camera** permissions.
3.  Select **Allow** (or Reset permission).
4.  **Reload** the page.

### 3. User Interaction Trigger
Most browsers require a distinct user gesture to activate the camera. Ensure you have explicitly clicked the **"Enable Webcam"** or **"Start Camera"** button on the interface. The permission popup may not trigger automatically on page load.

### 4. Check Hardware Availability
* Ensure no other application (e.g., Zoom, Microsoft Teams, Discord) is currently using the webcam.
* Verify the camera is physically connected and recognized by your OS.

### 5. Advanced Debugging
If the issue persists, open the **Developer Console** (`F12` or `Right Click -> Inspect -> Console`) and look for errors related to:
* `getUserMedia()`
...
**Still having issues?**
If you have followed all the steps above and cannot access the camera, please open an Issue with the following details:
* Browser Name & Version (e.g., Chrome 120.0)
* Operating System (e.g., Windows 11, macOS Sonoma)
* Connection Type (e.g., localhost vs. HTTPS deployment)